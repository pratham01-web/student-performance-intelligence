  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white">Student Performance Evaluator</h1>
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/predictions"
            className="inline-flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-800/80 px-3.5 py-2 text-xs font-semibold text-slate-300 hover:bg-slate-800 hover:text-white transition-colors"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 rounded-xl border border-slate-800 bg-slate-900/60 p-6 backdrop-blur-sm shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-6">
            <div>
              <h2 className="text-base font-semibold text-white">Student Academic &amp; Behavioral Parameters</h2>
              <p className="text-xs text-slate-400">Adjust parameters below to project expected performance</p>
            </div>
            <span className="text-[11px] font-medium text-slate-400 bg-slate-800 px-2.5 py-1 rounded">
              Input Form
            </span>
          </div>
          ...
          <form onSubmit={handlePredict} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1.5">Class / Grade Level</label>
                <select
                  value={formData.class_level}
                  onChange={(e) => handleSelectChange("class_level", e.target.value)}
                  className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-white focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                >
            ...
            <div className="space-y-2 pt-2 border-t border-slate-800/80">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-semibold text-blue-400">{formData.previous_marks.toFixed(0)}%</span>
              </div>
              <input
                type="range"
                min="30"
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-semibold text-indigo-400">{formData.study_hours.toFixed(1)} hrs/day</span>
              </div>
              <input
                type="range"
                min="0.5"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2 border-t border-slate-800/80">
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="text-slate-200 font-medium">{formData.sleep_hours}h</span>
                </div>
                <input
                  type="range"
                  min="4"
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="text-slate-200 font-medium">{formData.screen_time}h</span>
                </div>
                <input
                  type="range"
                  min="0.5"
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="text-slate-200 font-medium">{formData.study_days} days/wk</span>
                </div>
                <input
                  type="range"
                  min="1"
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={loading}
                className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-3 text-sm font-semibold text-white shadow-md hover:bg-blue-500 transition-colors disabled:opacity-50"
              >
        ...
        <div className="lg:col-span-5 space-y-6">
          {result && (
            <div className="rounded-xl border border-slate-800 bg-slate-900/80 p-6 backdrop-blur-sm shadow-md space-y-6">
              {/* Header */}
              <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                <div>
                  <span className="text-[11px] font-semibold text-blue-400 uppercase tracking-wider">Evaluation Report</span>
              ...
              <div className="rounded-lg border border-slate-800 bg-slate-950/80 p-4 space-y-3">
                <div className="flex items-baseline justify-between">
                  <span className="text-xs text-slate-400">Estimated Final Examination Score</span>
                ...
                <div className="flex items-center justify-between text-xs pt-1">
                  ...
                  <span className="text-slate-200 font-semibold">{result.grade_band}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-lg border border-slate-800 bg-slate-950/60 p-3.5">
                  <span className="text-[11px] text-slate-400 block mb-1">Pass Probability</span>
                ...
                <div className="rounded-lg border border-slate-800 bg-slate-950/60 p-3.5">
                  <span className="text-[11px] text-slate-400 block mb-1">Course Status</span>
                  ...
                  <span className="text-[10px] text-slate-500">Based on historical cutoffs</span>
                </div>
              </div>
              ...
              {result.key_factors && result.key_factors.length > 0 && (
                <div className="space-y-2">
                  <h4 className="text-xs font-semibold text-slate-300">Factor Contribution Breakdown</h4>
                  <div className="space-y-2">
                    {result.key_factors.map((f, i) => (
                      <div key={i} className="flex items-center justify-between rounded-lg border border-slate-800/80 bg-slate-950/40 px-3 py-2 text-xs">
                ...
                <div className="space-y-2.5 pt-2 border-t border-slate-800">
                  ...
                  <ul className="space-y-2">
                    {result.recommendations.map((rec, idx) => (
                      <li
                        key={idx}
                        className="text-xs text-slate-300 rounded-md border border-slate-800 bg-slate-950/40 p-2.5 leading-relaxed flex items-start gap-2"
                      >
