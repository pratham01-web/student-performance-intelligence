...
export default function StudentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (error || !student) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-16 text-center space-y-4">
        ...
        <Link
          href="/students"
          className="inline-flex items-center gap-2 text-xs text-neutral-300 hover:underline"
        >
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="rounded-xl border border-neutral-850 bg-black/60 p-6 backdrop-blur-md shadow-lg flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div className="flex items-center gap-4">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-neutral-900 border border-neutral-800 text-white font-bold text-xl">
            {student.name.charAt(0)}
          </div>
          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="text-2xl font-bold tracking-tight text-white">{student.name}</h1>
              <span className="font-mono text-xs px-2.5 py-0.5 rounded-full bg-neutral-900 text-neutral-200 border border-neutral-800">
        ...
        <div className="flex items-center gap-6 border-t md:border-t-0 md:border-l border-neutral-850 pt-4 md:pt-0 md:pl-6 text-xs text-neutral-400">
          <div>
            <div className="text-neutral-500">Enrolled On</div>
            <div className="font-semibold text-neutral-200 mt-0.5">
              {new Date(student.created_at).toLocaleDateString()}
            </div>
          </div>
          <div>
            <div className="text-neutral-500">Semesters Logged</div>
            <div className="font-semibold text-neutral-200 mt-0.5">
            ...
            <div className="divide-y divide-neutral-900">
              {student.academic_records.map((rec) => (
                <div key={rec.id} className="py-3.5 flex items-center justify-between text-xs">
                  <div>
                    <div className="font-semibold text-neutral-200">{rec.semester}</div>
                    <div className="text-neutral-400 mt-0.5 flex gap-3">
                      <span>Attendance: <strong className="text-emerald-400">{rec.attendance}%</strong></span>
                      <span>Prior Marks: <strong className="text-white">{rec.previous_marks}%</strong></span>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-neutral-500 text-[11px] block">Realized Score</span>
                ...
                <div key={habit.id} className="py-3.5 space-y-2 text-xs">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-neutral-300">
                    <div className="bg-neutral-950 p-2.5 rounded-lg border border-neutral-800">
                      ...
                      <div className="text-sm font-bold text-white mt-0.5">{habit.study_hours} hrs/d</div>
                    </div>
                    <div className="bg-neutral-950 p-2.5 rounded-lg border border-neutral-800">
                      <div className="text-[11px] text-neutral-500 flex items-center gap-1">
                        <Moon className="h-3 w-3" /> Sleep Time
                      </div>
                      <div className="text-sm font-bold text-neutral-200 mt-0.5">{habit.sleep_hours} hrs/d</div>
                    </div>
                    <div className="bg-neutral-950 p-2.5 rounded-lg border border-neutral-800">
                      <div className="text-[11px] text-neutral-500 flex items-center gap-1">
        ...
        {predictions.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-neutral-900 text-neutral-400 font-semibold uppercase text-[11px]">
                <tr>
                ...
                {predictions.map((p) => {
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td className="py-2.5 font-mono text-[11px] text-neutral-400">{p.model_version}</td>
                      ...
                      <td className="py-2.5">
                        <span
                          className={`px-2 py-0.5 rounded text-[11px] font-bold ${
                            isPass ? "bg-emerald-950 text-emerald-400 border border-emerald-800/60" : "bg-rose-950 text-rose-400 border border-rose-800/60"
                          }`}
                        >
                          {isPass ? "PASS" : "FAIL"}
                        </span>
